...
{#if $playerState && $playerState.playlist && $playerState.playlist.length > 0}
	<div class="playlist" class:open={isOpen}>
		<div
			...
			on:keypress={() => {}}
		>
			<button class="icon open" style:background-image={`url(${arrowIcon})`} />
			<button class="icon closed" style:background-image={`url(${playlistIcon})`} />
		</div>
		...
		<div
			class="tracks"
			use:dndzone={{ items: upcoming, flipDurationMs }}
