...
export function Music() {
    ...
    const guild = useSelector((state: RootState) => state.guilds);
    const playerState = useAppSelector((state: RootState) => state.musicPlayer.playerState);

    useEffect(() => {
        if (!guild.guildId) return;
        if (!playerState) {
            fetchPlayerState(guild.guildId);
        }
    });

    async function fetchPlayerState(guildId: string) {
        const playerState = await initPlayerState(guildId)
    ...
    if (guild.loading === 'idle' && guild.guildId === null) return (<div>Please select a Discord guild on the left pannel</div>);
    if (guild.guildId === undefined) return (<Loader />);
        ...
        <div className='music-container'>
            <div className='side-container'>
                ...
                <div className='results'>
                    {results?.map((r, i) => <div className='result' key={i} onClick={() => addToPlaylistClick(guildId, r)}>
