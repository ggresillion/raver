...
async function stream(url: string, options = {}, progress: (current: number) => void)
      ...
      if (canDemux) {
        ...
        const s = ytdl.downloadFromInfo(info, options)
          .on('response', res => {
            ...
            return resolve({stream: s, totalLengthSeconds: parseInt(info.length_seconds, 10)});
          })
          .pipe(demuxer)
